# Trivia questions for the Drive Co-Pilot game. Backed by the
# `trivia_questions` table — `SEED_POOL` below is the seed source, ported
# into rows by `rake trivia:seed`. `pick_for(person)` queries the DB and
# excludes questions the person has already answered correctly so kids
# don't see the same question twice.
module TriviaPool
  ...
  # Tag used for riddles in the shared trivia_questions table. Riddles aren't
  # an interest tag — they're a separate pool surfaced via the riddles mode
  # toggle on the Drive Co-Pilot picker.
  RIDDLE_TAG = "riddles".freeze

  GENERIC_RIDDLES = [
    { q: "I'm full of holes but still hold water. What am I?",
      options: [ "A net", "A sponge", "A sieve", "A cracked bucket" ], answer: 1,
      fun_fact: "Sponges trap water in their tiny pores." }
  ].freeze

  # Picks an unanswered question for the given person, scoped to their
  ...
  def self.pick_for(person)
    ...
    # the game keeps running. (No row written; this is a stopgap.)
    GENERIC.sample
  end

  # Picks an unanswered riddle for the given person. Riddles bypass the
  # interest-tag filter — they're a single shared pool keyed by tag
  ...
  def self.pick_riddle_for(person)
    ...
    GENERIC_RIDDLES.sample
  end

  # Coerces a TriviaQuestion row into the hash shape the controller/view
  # consume. Includes follow_up_id + chain metadata so the view can show
  # a "Continue the story →" CTA after a correct answer.
  def self.as_hash(record)
    {
      id: record.id,
      q: record.question,
      ...
      chain_intro: record.chain_intro,
      chain_position: record.chain? ? record.chain_position : nil,
      chain_length: record.chain? ? record.chain_length : nil,
      follow_up_id: record.follow_up&.id
    }
  end

  def self.has_interest?(tag)
    SEED_POOL.key?(tag.to_s)
  end
end
